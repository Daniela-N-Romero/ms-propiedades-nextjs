...
import PropuestaAdminTracker from "@/features/propuestas/propuesta-admin-tracker";
...
export default async function PropuestaDinamicaPage({ params }: Props) {
	...
	const propiedadesParaVista = propuesta.items.map((item) => {
		...
		return {
			...
			supCubierta: p.superficieCubierta
				? `${p.superficieCubierta} m²`
				: "Sin especificar",
			supTerreno: p.superficieTotal
				? `${p.superficieTotal} m²`
				: "Sin especificar",
			tiempoRuta: item.tiempoEstimadoString || "Ruta directa",
			ahorroTiempo: item.notaLogistica || "",
			aptitud: p.descripcion
				? `${p.descripcion.substring(0, 120)}...`
				: "Apto para logística y depósito",
	...
	const destinoLat = propuesta.puntoInteresLat
		? Number(propuesta.puntoInteresLat)
		: -34.6037;
	const destinoLng = propuesta.puntoInteresLng
		? Number(propuesta.puntoInteresLng)
		: -58.3816;

	return (
		<div className="min-h-screen bg-slate-50 flex flex-col">

      <PropuestaAdminTracker id={propuesta.id} />
				...
				<div className="max-w-7xl mx-auto flex flex-col md:flex-row justify-between items-start md:items-center gap-4">
					<div>
						...
						<p className="text-sm text-gray-300 mt-1">
							Presentado para:{" "}
							<strong className="text-white">{propuesta.clienteNombre}</strong>
						</p>
					</div>
